...
Patched copy of train_net.py, needed for two reasons (see Phase 0/1 audit
notes in docs/ and memory for the full history):

1. train_net.py's Trainer.build_train_loader() uses the *original*
   hierarchialdet.DiffusionDetDatasetMapper, which hardcodes two non-public
   noisy-box JSON paths in its __init__ and crashes on construction. This
   file uses hierarchialdet.dataset_mapper_patched.DiffusionDetDatasetMapper
   instead, which makes those paths optional via NOISY_BOX_TRAIN/NOISY_BOX_VAL
   env vars.
2. train_net.py's __main__ block hardcodes dataset registration to
   non-public paths ("../sorted/challenge/..."). This file requires
   TRAIN_JSON/TRAIN_IMG_DIR/VAL_JSON/VAL_IMG_DIR to be set in the
   environment instead (KeyError if not -- deliberately no silent default,
   since a wrong guessed path is worse than a loud failure). In practice
   these should point at the quadrant-enumeration-disease tier's real
   DENTEX paths -- the only tier the official loader can currently parse
   (see docs/phase1_dataset_audit.md for why quadrant/quadrant_enumeration
   fail).
"""
...
if __name__ == "__main__":
    ...
    # Dataset names ('custom_train_class' / 'custom_validation_class') must
    # match DATASETS.TRAIN / DATASETS.TEST in the config file being used
    # (e.g. configs/diffdet.custom.swinbase.nonpretrain.yaml). Paths come
    # from env vars rather than being hardcoded -- fails loudly (KeyError)
    # if unset, rather than silently defaulting to a path that may not
    # match the actual data layout on whatever machine this runs on.
    register_coco_instances('custom_train_class', {}, os.environ['TRAIN_JSON'], os.environ['TRAIN_IMG_DIR'])
    register_coco_instances('custom_validation_class', {}, os.environ['VAL_JSON'], os.environ['VAL_IMG_DIR'])
    launch(
        main,
        args.num_gpus,
